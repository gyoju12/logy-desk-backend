...
celery_app: Celery = fastapi_app.state.celery_app
...
async def process_document(self, document_id: str, file_path: str):
    ...
    try:
        ...
        db_document = await document.get(db, id=doc_uuid)
        ...
        for i, chunk in enumerate(chunks):
            ...
            db_chunk = await document_chunk.create(db, obj_in=chunk_data)
            await db.flush()
...
async def embed_chunk(self, chunk_id: str, chunk_content: str):
    ...
    try:
        ...
        db_chunk = await document_chunk.get(db, id=chunk_uuid)
